...
ReleaseConfigurationManifest = _release_manifest.ReleaseConfigurationManifest
...
def _validate_configuration_manifest(
    configuration: ReleaseConfigurationManifest,
    manifest: EmbeddedManifest,
) -> ReleaseConfigurationManifest:
    if type(configuration) is not ReleaseConfigurationManifest:
        raise SchemaError("configuration manifest must be strictly validated")
    try:
        canonical = ReleaseConfigurationManifest.from_mapping(
            configuration.to_mapping()
        )
    except (ReleaseManifestError, AttributeError, TypeError) as exc:
        raise SchemaError(
            "configuration manifest failed canonical revalidation"
        ) from exc
    if canonical != configuration:
        raise SchemaError("configuration manifest is not canonical")
    if (
        canonical.release_id != manifest.release_id
        or canonical.embedded_manifest_hash != manifest.manifest_hash
    ):
        raise EvidenceRejected(
            "configuration manifest does not match embedded manifest"
        )
    return canonical
...
class TargetSnapshotRequest:
    ...
    configuration_manifest: ReleaseConfigurationManifest
    ...
    def to_mapping(self) -> dict[str, Any]:
        return {
            ...
            "configuration_manifest": self.configuration_manifest.to_mapping(),
...
class TargetSnapshotReceipt:
    ...
    environment_delta_registry_digest: str
    ...
    def __post_init__(self) -> None:
        ...
        for field in (
            ...
            "environment_delta_registry_digest",
    ...
    def to_mapping(self) -> dict[str, Any]:
        return {
            ...
            "environment_delta_registry_digest": self.environment_delta_registry_digest,
...
class VerifiedTargetStateResolverPort(Protocol):
    def snapshot_verified(
        ...
        """Verify committed profiles/registry and actual target configuration.

        The request contains expectations, not proof. Independently resolve the
        immutable references, verify their contents/digests and approved deltas,
        and compare actual configuration before returning a complete snapshot.
        """
...
class TargetSnapshot:
    ...
    def _from_verified_receipt(
        ...
        object.__setattr__(
            snapshot, "configuration_manifest_hash", receipt.configuration_manifest_hash
        )
        object.__setattr__(
            snapshot,
            "environment_delta_registry_digest",
            receipt.environment_delta_registry_digest,
        )
        object.__setattr__(
...
def _resolve_target_snapshot(
    *,
    ...
    expected = {
        ...
        "candidate_artifact_digest": artifact.digest,
        "candidate_manifest_hash": manifest.manifest_hash,
        "configuration_manifest_hash": configuration_manifest.manifest_hash,
        "environment_delta_registry_digest": configuration_manifest.environment_delta_registry.digest,
        "target_profile_digest": (
            configuration_manifest.pilot.digest
            if target is TargetName.PILOT
            else configuration_manifest.production.digest
        ),
        "migration_class": manifest.migration_class,
    }
    for field, value in expected.items():
...
def _operation_from_verified_evidence(
    *,
    ...
    return ReleaseOperation(
        ...
        embedded_manifest_hash=manifest.manifest_hash,
        configuration_manifest_hash=target.configuration_manifest_hash,
...
class PromotionPlan:
    ...
    _configuration_manifest: ReleaseConfigurationManifest = dataclass_field(repr=False)
...
def plan_build_once_promotion(
    *,
    ...
    configuration_manifest = _validate_configuration_manifest(
    ...
    return PromotionPlan(
        ...
        _manifest=manifest,
        _configuration_manifest=configuration_manifest,
        _artifact=artifact,
...
def plan_rollback(
    *,
    ...
    previous_configuration_manifest: ReleaseConfigurationManifest,
    artifact_resolver: VerifiedArtifactResolverPort,
    ...
    configuration_manifest = _validate_configuration_manifest(
        previous_configuration_manifest, manifest
    )
    clock = _MonotonicTrustedClock(clock)
    artifact = _resolve_artifact(
        contract=contract,
...
def finalize_production_operation(
    *,
    ...
    configuration_manifest = _validate_configuration_manifest(
        plan._configuration_manifest, manifest
    )
    if (
...
def execute_operation(
    *,
    ...
    try:
        negative_matrix_receipt = ledger.negative_matrix_receipt(negative_matrix_query)
    except Exception as exc:
        LOGGER.warning(
            "negative-matrix receipt read failed closed (%s)", type(exc).__name__
        )
        raise LedgerSafetyError("cannot verify old auto-deploy is disabled") from exc
    now = _trusted_post_call_now(
        clock,
...
def preview_release(
    *,
    ...
    """Describe consistent declarations without resolving evidence or authorizing.

    This DTO cannot be deserialized into a trusted plan or executable operation.
    References are not fetched; even a declared digest still needs verification.
    """
    manifest = _validate_manifest(manifest, contract)
    configuration = _validate_configuration_manifest(configuration_manifest, manifest)
    artifact_digest = None
    if artifact_reference is not None:
        artifact_digest = _digest(
            artifact_reference.rsplit("@", 1)[-1], "preview.artifact_digest"
        )
        validate_immutable_image_reference(artifact_reference, artifact_digest)
        if artifact_reference != f"{contract.oci_repository}@{artifact_digest}":
            raise SchemaError("preview artifact repository does not match contract")

    missing_evidence = [
        "artifact_provenance_and_embedded_manifest",
        "trusted_ci_check_suite_and_exact_smoke",
        "configuration_references_and_delta_registry_contents",
        "delta_classification_and_pilot_decisions",
        "actual_pilot_and_production_configuration",
        "pilot_qualification",
        "independent_single_use_authority",
        "negative_matrix",
        "durable_ledger_and_claim",
        "provider_fencing_idempotency_and_reconciliation",
        "runtime_artifact_identity",
        "post_deploy_readiness_canary_and_data_invariants",
    ]
    if artifact_digest is None:
        missing_evidence.insert(0, "artifact_identity")
    result = {
        "schema_version": 1,
        "domain": "iwe.release-control.preview.v1",
        "status": "preview",
        ...
        "embedded_manifest": manifest.to_mapping(),
        "embedded_manifest_hash": manifest.manifest_hash,
        "configuration_manifest": configuration.to_mapping(),
        "configuration_manifest_hash": configuration.manifest_hash,
        "artifact": {"reference": artifact_reference, "digest": artifact_digest},
        "proposed_deployments": [
            {
                "target": target.value,
            ...
            for target, profile in (
                (TargetName.PILOT, configuration.pilot),
                (TargetName.PRODUCTION, configuration.production),
            )
        ],
        "missing_evidence": missing_evidence,
    }
    result["preview_digest"] = canonical_sha256(result)
    return result


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Inspect WP-562 release declarations")
    ...
    preview = subparsers.add_parser("preview-release")
    preview.add_argument("contract", type=Path)
    preview.add_argument("--embedded-manifest", type=Path, required=True)
    preview.add_argument("--configuration-manifest", type=Path, required=True)
    preview.add_argument("--artifact-reference")
    return parser


def _preview_from_files(
    args: argparse.Namespace, contract: ReleaseControlContract
) -> dict[str, Any]:
    try:
        embedded_payload = loads_strict_json(args.embedded_manifest.read_bytes())
        configuration_payload = loads_strict_json(
            args.configuration_manifest.read_bytes()
        )
    except OSError as exc:
        raise SchemaError("cannot read preview manifest inputs") from exc
    return preview_release(
        contract=contract,
        manifest=EmbeddedManifest.from_mapping(embedded_payload),
        configuration_manifest=ReleaseConfigurationManifest.from_mapping(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "preview-release":
            result = _preview_from_files(args, contract)
        else:
    ...
    except (ReleaseControlError, ReleaseManifestError) as exc:
        LOGGER.error("release-control input rejected: %s", exc)
        return 2
