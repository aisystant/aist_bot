...
называет понятие»» (утверждён пилотом 30.08, S0-правка 04.09). Наставник
(MIM.R.001, Режим 2) по запросу участника («объясни через понятия IWE», «какой
гейт мы применили») называет применённое системное понятие по-русски, код —
только в скобках после названия, и объясняет, зачем оно. Когда названное
...
class ConceptEntry:
    ...
    # (S0: «Название (код)», путь к файлу участнику не показывается).
    source_title: str
...
def format_concept_naming_section(lang: str = "ru") -> str:
    ...
    lines.extend([
        "ПРАВИЛА ОТВЕТА:",
        "1. Понятие называй по-русски, код — только в скобках после названия: «АрхГейт (AR.003)». "
        "Голый код или код вместо названия — ошибка.",
        "2. Источник цитируй как «название документа (код)». Пути к файлам, имена папок и "
        "репозиториев участнику НЕ показывай.",
