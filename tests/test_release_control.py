...
from scripts import release_control as release_cli
...
def _configuration_manifest(manifest, **overrides):
    def reference(path, digest):
        return {"repository_commit": SOURCE_COMMIT, "path": path, "digest": digest}
    ...
    payload = {
        ...
        "configuration_profiles": {
            "pilot": reference("profiles/pilot.json", DIGEST_D),
            "production": reference("profiles/production.json", DIGEST_E),
        },
        "environment_delta_registry": reference("profiles/deltas.json", DIGEST_F),
    }
    payload.update(overrides)
    return ReleaseConfigurationManifest.from_mapping(payload)
...
class FakeTargetResolver:
    ...
    def snapshot_verified(self, request):
        ...
        values = {
            ...
            "configuration_manifest_hash": request.configuration_manifest.manifest_hash,
            "environment_delta_registry_digest": request.configuration_manifest.environment_delta_registry.digest,
...
def _plan(
    ...
    *,
    ...
    embedded_manifest = manifest or _manifest(contract)
    return plan_build_once_promotion(
        contract=contract,
        manifest=embedded_manifest,
        configuration_manifest=(
            configuration_manifest
            if configuration_manifest is not None
            else _configuration_manifest(embedded_manifest)
        ),
...
def test_finalization_rejects_a_changed_production_profile_before_execution() -> None:
    contract = _contract(cutover=True)
    plan = _plan(contract)
    provider = FakeProvider()
    ledger = FakeLedger()
    ...
    with pytest.raises(EvidenceRejected, match="target_profile_digest"):
        operation = _qualified_production(
            contract,
            plan=plan,
            ledger=ledger,
            target_resolver=FakeTargetResolver(target_profile_digest=DIGEST_A),
        )
        _execute(
...
def test_planning_requires_the_manifest_profile_from_the_verified_target(
    target,
) -> None:
    contract = _contract(cutover=True)
    provider = FakeProvider()

    class ChangedProfile(FakeTargetResolver):
        def snapshot_verified(self, request):
            receipt = super().snapshot_verified(request)
            return (
                replace(receipt, target_profile_digest=DIGEST_A)
                if request.target is target
                else receipt
            )

    resolver = ChangedProfile()
    with pytest.raises(EvidenceRejected, match="target_profile_digest"):
        plan = _plan(contract, target_resolver=resolver)
        _execute(contract=contract, operation=plan.operations[0], provider=provider)

    assert resolver.requests[-1].target is target
    assert provider.calls == 0


@pytest.mark.parametrize(
    "field", ["configuration_manifest_hash", "environment_delta_registry_digest"]
)
def test_finalization_rejects_changed_configuration_evidence(field) -> None:
    contract = _contract(cutover=True)
    plan = _plan(contract)
    provider = FakeProvider()
    ledger = FakeLedger()
    ...
    with pytest.raises(EvidenceRejected, match=field):
        operation = _qualified_production(
            contract,
            plan=plan,
            ledger=ledger,
            target_resolver=FakeTargetResolver(**{field: DIGEST_A}),
        )
        _execute(
            contract=contract, operation=operation, ledger=ledger, provider=provider
        )
...
@pytest.mark.parametrize("field", ["release_id", "embedded_manifest_hash"])
def test_configuration_manifest_must_belong_to_the_candidate_before_resolving(
    field,
) -> None:
    contract = _contract(cutover=True)
    manifest = _manifest(contract)
    configuration = _configuration_manifest(
        manifest, **{field: "other-release" if field == "release_id" else DIGEST_A}
    )
    artifact = FakeArtifactResolver()
    with pytest.raises(EvidenceRejected, match="does not match embedded manifest"):
        _plan(
            contract, configuration_manifest=configuration, artifact_resolver=artifact
        )
    assert artifact.calls == 0


def test_a_raw_configuration_mapping_is_not_verified_evidence() -> None:
    contract = _contract(cutover=True)
    configuration = _configuration_manifest(_manifest(contract)).to_mapping()
    artifact = FakeArtifactResolver()

    with pytest.raises(SchemaError, match="strictly validated"):
        _plan(
            contract, configuration_manifest=configuration, artifact_resolver=artifact
        )
...
def test_finalization_rejects_replacement_of_any_planned_configuration_reference(
    reference,
) -> None:
    contract = _contract(cutover=True)
    plan = _plan(contract)
    original = plan._configuration_manifest
    changed = replace(
        original, **{reference: replace(getattr(original, reference), digest=DIGEST_A)}
    )
    object.__setattr__(plan, "_configuration_manifest", changed)
    qualification = FakePilotQualificationResolver()
    ...
    with pytest.raises(EvidenceRejected, match="sealed evidence linkage"):
        _qualified_production(
            contract,
            plan=plan,
            qualification_resolver=qualification,
...
def test_new_configuration_reference_invalidates_previous_operation_authority(
    reference,
) -> None:
    contract = _contract(cutover=True)
    first = _plan(contract)
    original = first._configuration_manifest
    # Even identical content at a different accepted commit is a new manifest.
    changed = replace(
        original,
        **{
            reference: replace(getattr(original, reference), repository_commit="3" * 40)
        },
    )
    second = _plan(contract, configuration_manifest=changed)
    operation = second.operations[0]
    provider = FakeProvider()
    ledger = FakeLedger()

    assert operation.artifact_digest == first.operations[0].artifact_digest
    assert operation.configuration_manifest_hash == changed.manifest_hash
    assert operation.operation_fingerprint != first.operations[0].operation_fingerprint
    with pytest.raises(AuthorityRejected, match="operation_fingerprint"):
        _execute(
            contract=contract,
            operation=operation,
            authority=_authority(first.operations[0]),
            provider=provider,
            ledger=ledger,
        )
    assert ledger.claim_calls == 0
...
def test_fresh_production_baseline_preserves_configuration_and_executes_with_new_authority() -> (
    None
):
    contract = _contract(cutover=True)
    plan = _plan(contract)
    ...
    resolver = FakeTargetResolver(
        current_artifact_digest=DIGEST_F,
        current_manifest_hash=DIGEST_C,
        target_config_digest=DIGEST_B,
        captured_at=NOW,
    )
    operation = _qualified_production(
    ...
    assert operation.expected_current_digest == DIGEST_F
    assert operation.target_config_digest == DIGEST_B
    assert operation.target_profile_digest == plan.operations[1].target_profile_digest
    assert (
        operation.configuration_manifest_hash
        == plan._configuration_manifest.manifest_hash
    )
    assert operation.target_snapshot_hash != plan.operations[1].target_snapshot_hash
    assert resolver.requests[0].configuration_manifest == plan._configuration_manifest
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"environment_delta_registry_digest": DIGEST_A},
    ],
)
def test_rollback_uses_the_previous_releases_fixed_configuration(overrides) -> None:
    contract = _contract(cutover=True)
    rollback = FakeRollbackResolver()

    with pytest.raises(
        EvidenceRejected,
        match="target_profile_digest|environment_delta_registry_digest",
    ):
        _rollback(contract, target=FakeTargetResolver(**overrides), rollback=rollback)
...
def _rollback(
    ...
    *,
    ...
    return plan_rollback(
        ...
        previous_manifest=embedded_manifest,
        previous_configuration_manifest=(
            configuration_manifest
            if configuration_manifest is not None
            else _configuration_manifest(embedded_manifest)
        ),
...
def preview_inputs(tmp_path):
    contract = load_release_control_contract(CONTRACT_PATH)
    manifest = _manifest(contract)
    configuration = _configuration_manifest(manifest)
    paths = {
    ...
    paths["embedded"].write_text(json.dumps(manifest.to_mapping()))
    paths["configuration"].write_text(json.dumps(configuration.to_mapping()))
    return paths


def _preview_arguments(paths, artifact_reference=None):
    args = [
        "preview-release",
        str(paths["contract"]),
        "--embedded-manifest",
    ...
    if artifact_reference is not None:
        args.extend(["--artifact-reference", artifact_reference])
    return args


@pytest.mark.parametrize("artifact_digest", [None, DIGEST_A])
def test_preview_is_deterministic_unverified_and_has_no_effects(
    preview_inputs, artifact_digest, capsys, monkeypatch
):
    def forbidden(*args, **kwargs):
        pytest.fail("preview must not resolve or execute anything")

    monkeypatch.setattr(release_cli, "plan_build_once_promotion", forbidden)
    monkeypatch.setattr(release_cli, "execute_operation", forbidden)
    monkeypatch.setattr(socket, "socket", forbidden)
    contract = load_release_control_contract(preview_inputs["contract"])
    reference = (
        f"{contract.oci_repository}@{artifact_digest}" if artifact_digest else None
    )
    args = _preview_arguments(preview_inputs, reference)
    original_files = {path: path.read_bytes() for path in preview_inputs.values()}

    assert release_cli.main(args) == 0
    output = capsys.readouterr().out
    preview = json.loads(output)
    assert preview["mutation_authorized"] is False
    assert preview["readiness"] == "not_verified"
    assert "independent_single_use_authority" in preview["missing_evidence"]
    assert "actual_pilot_and_production_configuration" in preview["missing_evidence"]
    assert preview["proposed_deployments"] == [
        {
            "target": "pilot",
        ...
        },
        {
            "target": "production",
            "artifact_digest": artifact_digest,
            "profile_digest": DIGEST_E,
        },
    ]
    assert ("artifact_identity" in preview["missing_evidence"]) is (
        artifact_digest is None
    )
    assert "operation_fingerprint" not in preview
    assert "_validated" not in output
    preview_digest = preview.pop("preview_digest")
    assert preview_digest == canonical_sha256(preview)
    assert {path: path.read_bytes() for path in original_files} == original_files
    assert set(preview_inputs["contract"].parent.iterdir()) == set(original_files)

    for path in preview_inputs.values():
        payload = json.loads(path.read_bytes())
        path.write_text(json.dumps(payload, sort_keys=True, indent=3))
    assert release_cli.main(args) == 0
...
@pytest.mark.parametrize(
    ...
    [
        ("embedded", "build_contract_digest", DIGEST_F),
        ("embedded", "required_checks", ["some-other-check"]),
        ("configuration", "release_id", "different-release"),
        ("configuration", "embedded_manifest_hash", DIGEST_F),
        ("configuration", "_validated", True),
        ("configuration", "approval", "pretend-approved"),
    ],
)
def test_preview_rejects_mismatched_or_injected_declarations_without_output(
    preview_inputs, document, field, value, capsys
):
    path = preview_inputs[document]
    payload = json.loads(path.read_bytes())
    payload[field] = value
    path.write_text(json.dumps(payload))

    assert release_cli.main(_preview_arguments(preview_inputs)) == 2
...
@pytest.mark.parametrize(
    "reference",
    [
        "registry.example.invalid/team/app:latest",
        f"different.example.invalid/team/app@{DIGEST_A}",
        f"registry.example.invalid/team/app@{SOURCE_COMMIT}",
    ],
)
def test_preview_rejects_mutable_or_unexpected_artifact(
    preview_inputs, reference, capsys
):
    assert release_cli.main(_preview_arguments(preview_inputs, reference)) == 2
    assert capsys.readouterr().out == ""


@pytest.mark.parametrize(
    "bad_input", ['{"schema_version":1,"schema_version":1}', "NaN", "{"]
)
def test_preview_uses_strict_json_parser(preview_inputs, bad_input, capsys):
    preview_inputs["configuration"].write_text(bad_input)
    assert release_cli.main(_preview_arguments(preview_inputs)) == 2
    assert capsys.readouterr().out == ""


def test_preview_missing_input_has_no_partial_output(preview_inputs, capsys):
    preview_inputs["configuration"].unlink()
    assert release_cli.main(_preview_arguments(preview_inputs)) == 2
    assert capsys.readouterr().out == ""


def test_preview_runs_as_isolated_cli(preview_inputs):
    result = subprocess.run(
        [
            sys.executable,
            "-I",
            "-B",
            str(REPO_ROOT / "scripts/release_control.py"),
            *_preview_arguments(preview_inputs),
        ],
        capture_output=True,
        text=True,
        check=False,
