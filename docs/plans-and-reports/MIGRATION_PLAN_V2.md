...
## ✅ Неделя 7: Очистка bot.py — ВЫПОЛНЕНО (частично)

**Цель:** Подготовить bot.py к миграции на State Machine путём выноса дублирующего кода в модули.

**Результат:** bot.py: 3850 → 3273 строк (-577 строк, -15%)

**Выполнено:**

1. ✅ **ClaudeClient удалён** (~410 строк)
   - Используется `from clients.claude import ClaudeClient`

2. ✅ **MCPClient** — уже в `clients/mcp.py`
   - Добавлен retry + увеличен таймаут до 30с

3. ✅ **init_db консолидирован** (~186 строк)
   - Используется `from db import init_db, get_pool`
   - db/models.py дополнен: fsm_states, feedback, schedule_time_2

4. ✅ **MCP timeout fix**
   - 10с → 30с + 1 retry

5. ✅ **i18n fixes**
   - Добавлены 3 недостающих ключа для es/fr

6. ✅ **qa_history table**
   - Добавлена в db/models.py

**Дополнительно выполнено (PR #251, #252):**

- ✅ **Вынос клавиатур** — `integrations/telegram/keyboards.py`
- ✅ **Settings button handling** — кнопка "⚙️ Настройки" работает во всех стейтах марафона
- ✅ **Day completion fix** — после завершения дня не показывается меню режимов

**Отложено (Phase 2):**

- 📋 **Вынос scheduler** (~100 строк)
- 📋 **Дальнейшая декомпозиция handlers**

**Проверка:**
- ✅ Syntax check: `python -m py_compile bot.py`
- ✅ Функциональное тестирование: /start, /learn, /mode, /progress, ?консультация
- ✅ MCP работает (10 результатов, фильтрация, дедупликация)
