...
|---|----------|-----------|--------|
| 1 | **Reply Keyboard сохраняется между стейтами** — кнопки "Да, давай сложнее" / "Достаточно" остаются видны после перехода в task.py | Средний | Открыта |

**Детали проблемы #1:**
- Reply Keyboard в Telegram сохраняется, пока не будет явно заменена или удалена
- `bonus.py.enter()` показывает Reply Keyboard с кнопками выбора
- `task.py.enter()` не убирает и не заменяет клавиатуру
- **Решение:** Каждый стейт должен либо показывать свою клавиатуру, либо вызывать `send_remove_keyboard()`

### Исправленные проблемы (2026-02-05)
...
| 4 | Ложное срабатывание кнопки "Да" в bonus.py (подстрока "да" в словах типа "создаёшь") | `4f2c314` |
