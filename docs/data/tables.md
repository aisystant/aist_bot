...
| `id` | SERIAL | — | PK |
| `chat_id` | BIGINT | — | NOT NULL |
| `started_at` | TIMESTAMPTZ | `NOW()` | Первый запрос сессии |
| `ended_at` | TIMESTAMPTZ | — | Последний запрос сессии; обновляется на каждом запросе |
| `duration_seconds` | INTEGER | — | Интервал `ended_at - started_at`, без последующего времени бездействия |
| `request_count` | INTEGER | `1` | Количество запросов между первым и последним запросом сессии |
...
Новая сессия начинается после разрыва не менее 30 минут от `ended_at`
последней сессии. Новые строки сразу получают `ended_at = started_at` и
`duration_seconds = 0`; поэтому они не требуют фоновой финализации. Старые
строки с `ended_at IS NULL` закрываются с нулевой известной длительностью:
время последнего запроса для них восстановить нельзя.
