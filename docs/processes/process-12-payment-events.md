...
**Надёжность приёма Telegram (WP-572).** Сообщение `successful_payment` проходит
ограничитель частоты и режим обслуживания: деньги уже списаны, поэтому событие
нельзя отбрасывать как обычное сообщение. Обработчик Мастерской ограничен своими
invoice payload; другие товары получают подтверждение в собственном обработчике.
Повтор записанного workshop charge не повторяет вызовы учёта и приглашение.
При недоставленном приглашении покупатель восстанавливает его через карточку
Мастерской или «Я оплатил — проверить». Это не заменяет долговечную очередь событий
при отказе процесса или БД; такие отказы требуют сверки с историей провайдера.
