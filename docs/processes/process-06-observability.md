...
Для запросов онбординга ожидание соединения измеряется отдельными интервалами
`*.pool_acquire`. Поэтому медленный `/start` можно разделить на ожидание пула,
сам запрос и внешние вызовы. Алерт о трёх медленных навигационных командах не
называет причину заранее: причину подтверждают интервалы запроса, ожидание
соединения и журнал холодного старта.
...
| Дата | Изменение |
|------|-----------|
| 2026-08-06 | WP-330 Ф16: ожидание соединения выделено в `pool_acquire`; алерт навигации больше не приписывает задержку пулу без доказательств |
