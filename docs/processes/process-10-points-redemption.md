...
## 8. `skip_ceiling` — какие покупки его используют (WP-446, обновлено 2026-07-03)
...
| Семинар (`workshop.py`, `showcase.py`) | `True` | 2026-07-03, пилот попросил распространить и на семинары |

**Побочный эффект (устранён 2026-07-03):** `handlers/points.py` строка «Доступная скидка: ~X ₽» (`balance × rate`, без применения потолка вообще) теперь точна для всех трёх видов покупок — потолок нигде не ограничивает per-transaction сумму, только общий баланс `point_balances.points`.

**YooKassa-модель отличается от cashback:** для семинаров скидка уменьшает сумму реального платежа в ЮКассе (`payable_rub = requested_amount_rub - discount_rub`, §2 выше) — в отличие от подписки/курсов, где полная цена списывается в Aisystant, а бонусы уходят отдельным списанием после подтверждения. `available_discount` уже ограничивает `available_pts` через `max_by_request = requested_amount_rub / rate` независимо от `skip_ceiling` — оплата не может уйти в минус, но при большом балансе бонусов может целиком покрыть недорогой семинар (`payable_rub = 0`).
