...
- **Л2.2 tool-call audit** (ArchGate 2026-07-07, privacy v2 — WP-544 Д6.5): `db.queries.traces.log_tool_call_audit()` пишет в `domain_event` (`event_type='tool_call_audit'`, `schema_version='v2'`) обезличенные метаданные каждого вызова через `tool_executor` (`question_handler.py`), fire-and-forget. Сохраняются количество доступных tool, стабильные SHA-256-отпечатки доступных и выбранного tool, число ключей входа и длины вопроса, входа и результата; сами имена tool и ключей, текст вопроса, значения входа, результат и Telegram ID не сохраняются. `external_id` — непрозрачный UUID, субъект записывается только в колонку `account_id`, чтобы работали RLS и удаление данных; если канонический `account_id` ещё недоступен, событие не записывается. Этого достаточно для корреляции регресса выбора с изменением набора tool, но не для чтения содержимого обращения. Изменение действует только на новые события; очистка исторических строк требует отдельной процедуры с учётом append-only hash-chain.
