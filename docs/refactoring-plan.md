...
## Статус 2026-04-11 (ревизия)

Документ создан в январе 2026 для миграции с монолитной `bot.py` архитектуры. За 2.5 месяца многое изменилось, часть плана устарела.

### ✅ Сделано и неактуально

- **Фаза 1 Документация** — выполнено, `ontology.md` + CLAUDE.md §2 живут
- **Монолитная `bot.py`** больше не существует. Рефакторинг разнёс её на `core/`, `handlers/`, `states/`, `engines/`. Ссылки на `bot.py:~259-262` и аналогичные строки в фазах 2-6 — **нерелевантны**
- **Фазы 3-6 БД-миграция** — **осознанно отложена**. Таблица `interns` была мигрирована в `public.users` + `development.user_state` (WP-82 Ф3), но без переименования полей `bloom_level` / `feed_sessions`. Решение: ломать DB-схему только при реальной боли от терминологии, а не ради consistency
- **`ontology.md` как source-of-truth** работает: CLAUDE.md §2.2 содержит mapping «в коде сейчас» → «целевое имя» как living reference, без обязательной миграции

### 🔄 Актуальное (осталось)

Переименование полей и функций откладывается до момента, когда это станет необходимо (например, при крупном рефакторе FSM states). Минимальный жест — **обновлять UI-строки** в i18n (ru/en/es/fr), чтобы пользователь видел правильные термины («урок», «задание», «дайджест», «фиксация»), даже если в коде остаются `theory`/`practice`/`feed_session`.

### ❌ Архивировано (не делать)

- Массовое переименование `bloom_level` → `complexity` — не нужно, `bloom_level` точнее описывает семантику (Bloom's taxonomy)
- `answer_type='theory_answer' → 'lesson_answer'` — миграция DB enum болезненна, а внутреннее имя не видно пользователю
- `feed_sessions` → `digests` — таблица используется в слишком многих запросах, цена рефактора > выгоды

### Что ещё стоит добавить

- **Единый glossary в Pack** (DP.AISYS.014 уже содержит сценарии + FAQ, нужна отдельная секция «термины кода → термины пользователя»)
- **Linter для i18n** — чтобы новые строки не вносили «теорию» и «сессии» в UI

---

## Исторический план (ниже — для контекста)

> Текст ниже — оригинальный план января 2026. Сохранён для справки и истории, **не является руководством к действию** в 2026-04.
