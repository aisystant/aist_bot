...
[Ссылка t.me/aist_me_bot?start=masterskaya_direct]
     ...
     │        да → повторно выдать приглашение в Мастерскую (конец)
     │        нет ↓
...
- Повторный `/start masterskaya_direct`, старая кнопка оплаты и «Я оплатил — проверить»
  восстанавливают приглашение в Мастерскую при уже имеющемся доступе. Новый платёж
  не создаётся; одна прямая покупка не маршрутизируется в чат Семинара.
- Перед списанием Stars проверяются 4000 XTR, покупатель в параметре счёта и
  отсутствие уже оплаченного доступа. Ошибка проверки доступа отклоняет списание.
- Подтверждение успешной оплаты проходит ограничитель частоты и режим обслуживания.
  Обработчик Мастерской принимает только свои счета; подтверждения других товаров
  продолжают маршрутизацию. Повтор того же charge_id не повторяет приглашение и учёт.
- Для рабочей базы обязательна миграция 046 (`workshop_payments.product TEXT NULL`).
  При `SKIP_DB_MIGRATIONS=true` её применяет владелец схемы до выпуска; проверка
  готовности платежей останавливает запуск при отсутствии поля. Старые записи
  остаются с `NULL`, автоматического присвоения прямого товара нет.
...
| 2026-09-25 | WP-572: проверка схемы оплаты, восстановление приглашения, защита платёжных событий от фильтров, проверка прямого счёта и отсутствие повторных действий на дубликате. |
