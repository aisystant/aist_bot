...
> ⚠️ Пункт 3 (старт-параметр `consent`) — пока **не реализован**. Backlog (см. WP-188 или новый РП). Без него рассылка ведёт в обычный /start → пилот должен явно набрать `/consent opt-in`.
