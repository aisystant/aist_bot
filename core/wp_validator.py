...
Правило: РП начинается с существительного, обозначающего ДОКУМЕНТ, ВЕЩЬ
или СОСТОЯНИЕ СИСТЕМЫ. НЕ с глагола, процесса, метода или желания.
Отглагольные существительные-процессы (анализ, исследование, обзор,
диагностика, сравнение) — ЗАПРЕЩЕНЫ как начало РП.
...
# Белый список: артефактные существительные в начале (документ, вещь, состояние)
GOOD_STARTS = {
    'чек-лист', 'список', 'таблица', 'схема', 'план', 'текст', 'пост',
    'описание', 'определение', 'формулировка', 'карта', 'матрица',
    'диаграмма', 'набор', 'реестр', 'отчёт',
    'протокол', 'шаблон', 'черновик', 'заметка', 'конспект', 'выписка',
    'перечень', 'классификация', 'прогноз',
}

# Стоп-слова: отглагольные существительные-процессы (НЕ артефакты).
# Начало с этих слов → Level 2 не спасёт, сразу bad.
BAD_PROCESS_NOUNS = {
    'анализ', 'исследование', 'обзор', 'диагностика', 'сравнение',
    'обогащение', 'масштабирование', 'интеграция', 'публикация',
    'развёртывание', 'создание', 'разработка', 'проектирование',
    'тестирование', 'моделирование', 'оценка',
}
...
def validate_formulation_regex(text: str) -> dict:
    ...
    # Процессные существительные — точно плохо (документ, вещь, состояние — НЕ процесс)
...
Формулировка ДОЛЖНА начинаться с существительного, обозначающего ДОКУМЕНТ, ВЕЩЬ или СИСТЕМУ В ОПРЕДЕЛЁННОМ СОСТОЯНИИ.
Тест: можно ли это увидеть, передать или показать?

ЗАПРЕЩЕНО начинать с:
- Глаголов: «Проанализировать», «Сделать», «Внедрить»
- Процессов: «Работа над...», «Процесс публикации»
- Методов: «Метод анализа», «Способ решения»
- Желаний: «Хочу сделать...», «Надо...»
- Отглагольных существительных-ПРОЦЕССОВ: «Анализ...», «Исследование...», «Обзор...», «Диагностика...», «Сравнение...»

ХОРОШО: «Чек-лист привычек», «Описание текущего состояния», «Таблица целей», «Схема процесса», «Текст поста»
ПЛОХО: «Анализ различий», «Исследование подходов», «Обзор методов», «Диагностика системы»

Ответь СТРОГО в формате:
VALID: true или false
SUGGESTION: <если false — предложи исправленную формулировку, начинающуюся с документа/вещи>"""
