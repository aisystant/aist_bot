...
## ✅ Неделя 6: Интеграция Claude API в State Machine — ВЫПОЛНЕНО

**Выполнено:**

| Файл | Метод Claude | PR |
|------|--------------|-----|
| `states/workshops/marathon/lesson.py` | `claude.generate_content()` | #210 |
| `states/workshops/marathon/question.py` | `claude.generate_question()` | #210 |
| `states/workshops/marathon/task.py` | `claude.generate_practice_intro()` | #210 |
| `bot.py` — routing `/learn` → State Machine | `go_to()` | #211 |
| `core/machine.py` — метод `go_to()` | — | #211 |
| `core/knowledge.py` — модуль работы с темами | — | #210 |

**Добавлены i18n ключи:**
- `marathon.generating_question`, `marathon.no_topics_available`
- `marathon.task_default`, `marathon.work_product_default`
- `errors.content_generation_failed`, `errors.question_generation_failed`, `errors.try_again_later`

**Проверка:**
- [ ] `/learn` с `USE_STATE_MACHINE=true` показывает сгенерированный контент
- [ ] Smoke test: генерация урока < 30 секунд

---

## 🔴 Неделя 7: Очистка bot.py — ТЕКУЩАЯ

**Цель:** Подготовить bot.py к миграции на State Machine путём выноса дублирующего кода в модули.

> Подробный план: `docs/plans/cleanup-bot-plan.md`

**Проблема:** bot.py = 3875 строк монолитного кода, включая:
- ClaudeClient (410 строк) — дублирует `clients/claude.py`
- MCPClient (160 строк) — дублирует `clients/mcp.py`
- БД-код (330 строк) — дублирует `db/`
- Клавиатуры (120 строк) — нужно вынести
- Scheduler (190 строк) — нужно вынести

**Задачи:**

1. **Интеграция с clients/** (~570 строк)
   - Удалить ClaudeClient из bot.py
   - Удалить MCPClient из bot.py
   - Использовать `from clients import claude, mcp_guides, mcp_knowledge`

2. **Интеграция с db/** (~330 строк)
   - Удалить дублирующий код БД
   - Использовать `from db import init_db` и `from db.queries import ...`

3. **Вынос клавиатур** (~120 строк)
   - Создать `keyboards.py` или `ui/keyboards.py`
   - Перенести все `kb_*` функции

4. **Вынос scheduler** (~190 строк)
   - Создать `scheduler/` или `engines/scheduler.py`
   - Перенести функции планировщика

5. **Консолидация config** (~65 строк)
   - Убедиться, что все константы из `config/`

**Проверка:**
- bot.py < 2700 строк
- Smoke test: `python -c "from bot import dp, bot"`
- Все существующие тесты проходят
- Compliance checker зелёный

**Связь с миграцией:**
После очистки handlers готовы к разбиению на `states/`.
...
### Авторизация через Ory (OIDC/OAuth2)

**Проблема:** MCP-сервер требует авторизации. Telegram-бот не имеет браузера.

**Предлагаемый flow:**

```
1. Пользователь: /connect или первый запуск
2. Бот: генерирует уникальный state + PKCE code_verifier
3. Бот: отправляет ссылку на Ory Login:
   https://auth.aisystant.com/oauth2/auth?
     client_id=aist_bot
     &redirect_uri=https://bot.aisystant.com/callback
     &response_type=code
     &scope=openid profile digital_twin
     &state={telegram_user_id}:{nonce}
     &code_challenge={code_challenge}
     &code_challenge_method=S256

4. Пользователь: открывает ссылку в браузере
5. Ory: показывает форму логина/регистрации
6. Пользователь: авторизуется
7. Ory: редиректит на callback с ?code=...&state=...
8. Callback endpoint (FastAPI в боте):
   - Валидирует state
   - Обменивает code на access_token + refresh_token
   - Сохраняет токены в БД (зашифрованно)
   - Уведомляет бота через внутренний канал
9. Бот: отправляет пользователю "✅ Авторизация успешна!"
10. Все последующие запросы к MCP идут с Bearer token
```

**Критика текущего flow:**

| Аспект | Проблема | Решение |
|--------|----------|---------|
| Безопасность | state может быть перехвачен | Использовать PKCE + короткий TTL |
| UX | Переход в браузер неудобен | Добавить Telegram Login Widget как альтернативу |
| Refresh | Токены истекают | Автоматический refresh в middleware |
| Fallback | Если Ory недоступен | Работать с локальной БД |

**Альтернативы:**

1. **Telegram Login Widget** — если Ory поддерживает
2. **Device Authorization Flow** — показываем код, пользователь вводит на сайте
3. **Magic Link** — отправляем email со ссылкой авторизации

**Проверка:**
- [ ] `/connect` открывает ссылку авторизации
- [ ] После авторизации бот получает уведомление
- [ ] Состояние синхронизируется с Digital Twin
- [ ] При недоступности MCP — fallback на PostgreSQL
...
**Статус:**
- ✅ `states/utilities/progress.py` — создан
- 📋 `states/utilities/notes.py` — в плане
- 📋 `states/utilities/export.py` — в плане
...
3. ~~Создать `states/utilities/progress.py`~~ ✅ Выполнено

4. Создать `integrations/export/` с адаптерами (Obsidian, Notion, Markdown)

5. Настроить глобальные команды `/note`, `/export`
...
**Цель:** Финальная очистка и стабилизация.
...
| **Очистка bot.py** | **7** | **🔴 ТЕКУЩАЯ** |
| Digital Twin MCP | 7.5 | 📋 В плане |
| Утилиты | 8 | 📋 Частично (progress.py ✅) |
| Тест ступени | 9 | 📋 В плане |
| Стабилизация | 10 | 📋 В плане |
| Новые мастерские | 11 | 📋 В плане |
| Mini App | 12 | 📋 В плане |

## Текущие возможности
- ✅ State Machine инфраструктура работает с feature flag
- ✅ Марафон (State Machine): lesson → question → bonus → task — **Claude API интегрирован**
- ✅ Марафон (bot.py): полностью работает при `USE_STATE_MACHINE=false`
- ✅ Лента: topics → digest — работает через `engines/feed/`
- ✅ Консультация: глобальный вызов через `?`
- ✅ i18n: 4 языка (ru/en/es/fr), 240+ ключей
- ✅ Генерация контента на выбранном языке (во всех режимах)

## Будущее

### Digital Twin MCP (Неделя 7.5)
- **Endpoint:** `https://digital-twin-mcp.aisystant.workers.dev/mcp`
- **Репозиторий:** `github.com/aisystant/digital-twin-mcp`
- **Интеграция:** Через `core/storage.py` → `DigitalTwinStorage`
- **Авторизация:** Ory OIDC/OAuth2 с PKCE flow

### Ory Identity
- Проверка прав при переходах между стейтами
- Интеграция с Digital Twin авторизацией

### Mini App (Неделя 12)
- Расширенный UI для профиля, тем и статистики
- Telegram Mini Apps SDK + FastAPI backend
