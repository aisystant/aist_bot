...
## 🔴 Неделя 6: Интеграция Claude API в State Machine — ТЕКУЩАЯ

**Проблема:** Стейты Марафона содержат TODO-заглушки, генерация контента НЕ РАБОТАЕТ.

```
# Пример заглушки в states/workshops/marathon/lesson.py:96
# TODO: Интеграция с claude.generate_content()
```

**Почему это критично:**
- При `USE_STATE_MACHINE=true` бот показывает заглушки вместо сгенерированного контента
- Лента работает (использует `engines/feed/`), а Марафон — нет
- Нельзя тестировать State Machine без рабочей генерации

**Задачи:**

| Файл | TODO | Метод Claude | Статус |
|------|------|--------------|--------|
| `states/workshops/marathon/lesson.py` | строка 96 | `claude.generate_content()` | 📋 |
| `states/workshops/marathon/question.py` | строка 82 | `claude.generate_question()` | 📋 |
| `states/workshops/marathon/task.py` | строка 79 | `claude.generate_practice_intro()` | 📋 |
...
2. Получить данные темы из `knowledge_structure.yaml`

3. Вызвать генерацию и отправить результат
...
## 📋 Неделя 7: Очистка bot.py (после интеграции)
...
| **Интеграция Claude API** | **6** | **🔴 ТЕКУЩАЯ** |
| Очистка bot.py | 7 | 📋 В плане |
| Утилиты | 8 | 📋 В плане |
...
- ⚠️ Марафон (State Machine): lesson → question → bonus → task — **TODO-заглушки, генерация не работает**
...
- ✅ i18n: 4 языка (ru/en/es/fr), 236+ ключей
- ✅ Генерация контента на выбранном языке (в bot.py и engines/feed/)
