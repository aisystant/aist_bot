...
| [SC.008](../../PACK-digital-platform/pack/digital-platform/08-service-clauses/DP.SC.008-self-healing.md) | Самовосстановление | S31 L1 Unstick, S32 L2 Auto-Fix, L3 detect + intervention alert, S34 L4 Escalate; S33 L3 Restart — только внешний защищённый контур |
...
Граница WP-562: процесс бота не реализует и не уполномочен выполнять S33 L3
Restart. Обещание DP.SC.008 «сервис восстановлен < 2 мин» остаётся открытым до
подключения независимого запускателя с ограниченными правами, журналом решения
и точной целью. До этого живой выпуск не может считать S33 выполненным.
...
### 4.1. Ежедневная разработка (pilot, только до cutover)

**Правило Pilot-First:** Все изменения по умолчанию → ветка `pilot`. Решение
пользователя о выпуске запускает только управляемое продвижение из §4.2 и не
является разрешением на прямой push в `new-architecture` (production).
...
Этот источник деплоя действует только пока `cutover.enabled=false`. Включение
§4.2 атомарно отключает branch/source auto-deploy для обеих сред: после
переключения ни push в `pilot`, ни push в `new-architecture` не может изменить
запущенный digest.

### 4.2. Управляемое продвижение в production (WP-562)

**Статус перехода:** непродовый контур реализуется с `cutover.enabled=false`.
Пока серверные правила, отдельная одноразовая полномочность и внешний журнал не
приняты живыми отрицательными тестами, production-мутация остановлена. Прямой
push, повторная сборка и ручной перенос коммитов не являются запасным путём.

**Предусловие cutover:** у обеих Railway-сред отключена GitHub/branch source
integration, единственным источником деплоя служит точный OCI digest из
разрешённого registry. Живой отрицательный тест обязан доказать, что push в
каждую из двух веток не меняет digest ни одной среды; квитанция теста входит во
внешний журнал до первой production-мутации.

Это предусловие — не только формулировка, а машинно принудительный gate
(`scripts/release_control.py`) из двух разных по силе слоёв, и их нельзя
путать. **Статический слой** (тот же валидатор контракта, что гоняется в CI,
`release-control-dry-run.yml`) при `cutover.enabled: true` требует непустой,
синтаксически корректный `cutover.negative_matrix_evidence_ref` — но проверяет
только форму строки, не её правдивость: он не обращается к журналу и не может
отличить настоящую ссылку на квитанцию от придуманной. **Runtime-слой**
(`execute_operation()`, вызывается только реальным запускателем продвижения) —
единственная содержательная проверка: точное совпадение репозитория,
контракт-дайджеста, обеих веток и свежесть по TTL
(`negative_matrix_max_age_seconds`, по умолчанию 24ч) против записи в журнале
(`ReleaseLedgerPort.negative_matrix_receipt`). Без действительной свежей
квитанции продвижение падает до захвата claim — ни pilot, ни production не
мутируются.

**Что реально блокирует production сегодня** — не эта пара слоёв, а два
независимых от неё факта: (1) в закоммиченном контракте `cutover.enabled:
false`; (2) CI (`release-control-dry-run.yml`) отдельно вызывает
`validate-contract --require-cutover-disabled`, что явно требует `enabled:
false` независимо от содержимого `negative_matrix_evidence_ref`. Эта
CI-проверка существовала до текущей правки и не относится к негативной
матрице — упомянута здесь, чтобы не создавать ложное впечатление, будто
именно формат-проверка `negative_matrix_evidence_ref` — единственное, что
сегодня держит cutover выключенным.

**Открытый follow-up (вне этой правки):** ни сама квитанция (живой запрос к
Railway API или ручная attestation пилота, подтверждающая отключение старого
канала), ни боевая реализация `ReleaseLedgerPort`, которая бы её хранила и
отдавала, пока не существуют — весь запускатель продвижения остаётся
библиотекой без подключённого журнала. Runtime-слой проверки от этого не
становится бесполезным: он готов к моменту, когда журнал появится, и не даст
включить cutover без настоящей проверенной квитанции. Отдельный, ещё не
принятый вопрос для этого будущего механизма — нужна ли независимость
утверждающего (тот же принцип, что уже применён к `pilot_qualification_*` в
контракте, а не тот же человек, что инициирует cutover).

**Триггер после включения:** отдельное решение пилота о продвижении уже
проверенного кандидата.

**Штатное действие после включения:**

1. CI один раз собирает `linux/amd64` образ из точного дерева и фиксирует OCI manifest digest.
2. Обязательные проверки подтверждают контракт, безопасность, полный regression и смысловое соответствие веток.
3. Пилот получает этот digest; `/attest`, `/ready`, canary-сценарий и инварианты данных формируют квитанцию.
4. Независимый владелец решения выдаёт одноразовое ограниченное по времени разрешение на точную цель и digest.
5. Единственный запускатель сначала фиксирует claim с версией и fencing token, затем продвигает в production тот же digest без rebuild.
6. После выпуска повторяются точная аттестация, готовность, canary и проверки данных; результат записывается во внешний append-only журнал.
7. Неизвестный исход не повторяется вслепую: независимый наблюдатель сначала сверяет фактическое состояние, а неоднозначность оставляет цель заблокированной.
8. Откат использует сохранённый предыдущий digest без rebuild и требует свежей проверки доступности и совместимости схемы.

**Аварийная граница:** исключение должно быть записано и независимо разрешено
до мутации. Пока этот путь не реализован и не проверен, отказ release-control
означает STOP, а не разрешение прямого push.

**Выход:** production указывает на тот же проверенный digest, а выпуск
восстанавливается по машинной квитанции без ручного сравнения истории.
