...
async def get_users_for_practice_nudge(limit: int = 100) -> list[dict]:
    """WP-330 Ф10.D: получить пользователей, которые получили урок сегодня
    (>30 мин назад), но не нажали «✏️ Перейти к практике».

    Возвращает sent_at, чтобы планировщик мог определить окно:
    - 30–150 мин → первый нудж (:30m)
    - >150 мин   → второй нудж (:150m)

    Не напоминать, если пользователь уже сделал чек-ин за этот день.
    """
    pool = await get_learning_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(
