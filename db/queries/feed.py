...
async def create_feed_week(chat_id: int, suggested_topics: List[str] = None,
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow('''
            ...
            FROM public.feed_weeks WHERE chat_id = $1
        ''', chat_id)
        week_number = row['next_week']

        result = await conn.fetchrow('''
            INSERT INTO public.feed_weeks
...
async def update_feed_week(week_id: int, updates: dict):
    ...
    async with pool.acquire() as conn:
        for key, value in updates.items():
            ...
            await conn.execute(
                _update_sql('public.feed_weeks', [key], 'id = $1'),
                week_id, value
            )
...
async def update_feed_session(session_id: int, updates: dict):
    ...
    async with pool.acquire() as conn:
        for key, value in updates.items():
            ...
            await conn.execute(
                _update_sql('public.feed_sessions', [key], 'id = $1'),
                session_id, value
            )
...
async def get_feed_session_by_id(session_id: int) -> Optional[dict]:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            'SELECT * FROM public.feed_sessions WHERE id = $1',
            session_id
        )
...
async def get_feed_session(week_id: int, session_date: date) -> Optional[dict]:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            '''SELECT * FROM public.feed_sessions
    ...
    async with pool.acquire() as conn:
        if keep_completed:
            deleted = await conn.execute(
                "DELETE FROM public.feed_sessions WHERE week_id = $1 AND status NOT IN ('completed', 'skipped', 'expired')",
                week_id
            )
        else:
            deleted = await conn.execute(
                'DELETE FROM public.feed_sessions WHERE week_id = $1',
                week_id
            )
...
async def get_feed_history(chat_id: int, limit: int = 10) -> List[dict]:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch('''
            ...
            JOIN public.feed_weeks w ON s.week_id = w.id
...
async def get_feed_session_content(session_id: int) -> Optional[dict]:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            'SELECT id, content, topic_title, fixation_text, session_date, status FROM public.feed_sessions WHERE id = $1',
            session_id
        )
        if row:
            return {
