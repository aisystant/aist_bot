...
_SPEND_KEYBOARD = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="🎓 Курсы с наставником", callback_data="sched_pay_courses")],
])
...
async def cmd_points(message: Message):
    ...
    if not events:
        text += (
            ...
            "Закрывай день /day_close, делай занятия, фиксируй слоты саморазвития.</i>"
        )
    else:
...
async def cmd_rules(message: Message):
    ...
    text = (
        ...
        "-- Содержательные действия (занятия, задания, публикации): "
        ...
        "-- Друг оформил платную подписку по твоей ссылке — начислится 3000 бонусов\n"
        "-- Сумма фиксированная, не зависит от твоей квалификации\n"
        "Своя ссылка — /referral\n"
        "\nСвой баланс — /points"
    )
