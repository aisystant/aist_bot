...
async def cmd_delivery_canary(message: Message):
    ...
    Проверяет полный цикл: enqueue → очередь → drain (до 1 мин) → журнал.
    Шаг 1: вставить запись в notification_queue.
    Шаг 2: показать текущее состояние очереди.
    Шаг 3: через 65 сек проверить, что запись доставлена (drain сработал).
    """
    if not _is_developer(message.chat.id):
        return
    ...
    await message.answer("🔧 <b>Canary WP-418 Доставщик</b>\nШаг 1: вставляю тестовое сообщение в очередь…", parse_mode="HTML")
    ...
    icon = "✅" if ok else "⚠️"
    verdict = "PASS — drain доставил" if final_status == "delivered" else (
        "PASS — подавлен (journal-dup, ожидаемо при повторе)" if final_status == "suppressed" else
        f"FAIL — статус остался '{final_status}' (drain не сработал?)"
    )
