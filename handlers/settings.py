...
async def cmd_connect_guide(message: Message):
    ...
    await message.answer(
        ...
        "• Завтрашнее занятие учтёт сегодняшний результат\n\n"
        "После подключения тебе станут доступны личные заметки в репозитории "
        "и помощник для планирования дня — это следующий шаг после подписки.",
        parse_mode="HTML",
