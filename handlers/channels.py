...
async def _generate_draft(
    ...
    # 5. Thinking tools (FPF-различения для содержательных ответов)
    thinking_tools = (channel_context or {}).get('thinking_tools', [])
    if 'fpf_distinctions' in thinking_tools:
        system_parts.append(
            "\nИнструменты мышления (используй для анализа рассуждений собеседника):"
            "\n- Целевая система vs эволюционная: у спроектированной системы (мост, компания) "
            "цель назначена создателем (constructor). У эволюционной (биосфера, цивилизация) "
            "есть наблюдаемые функции, но нет «назначенной цели». Приписывать второй логику первой — "
            "категориальная ошибка (телеологическое мышление)."
            "\n- Объект ≠ модель: свойства системы и наша модель системы — разные вещи. "
            "«Цель» может быть свойством модели наблюдателя, а не самой системы."
            "\n- Аксиологическая нейтральность: нет встроенной иерархии ценностей. "
            "Цели не «обнаруживаются» — они ставятся явно, на основании наблюдений и понимания."
            "\n- Эмерджентность: свойства целого не сводятся к сумме частей. "
            "Порядок может возникать без замысла — через отбор и адаптацию."
            "\n- Роль контекстуальна: у элемента системы нет «предназначения» — "
            "есть роли, которые зависят от контекста (надсистемы, наблюдателя)."
            "\nПрименяй эти различения, когда собеседник делает неявные допущения в рассуждениях. "
            "Указывай на допущение уважительно — не «вы ошибаетесь», а «в вашем рассуждении "
            "есть неявная предпосылка: [какая]. Если её убрать, цепочка меняется»."
        )

    # 6. Правила
    system_parts.append(
        "\nПравила:"
        "\n- НИКОГДА не пересказывай и не перефразируй входящее сообщение"
        "\n- Если сообщение — вопрос: дай содержательный ответ по существу"
        "\n- Если сообщение — рассуждение или комментарий (не вопрос): дай содержательную реакцию — "
        "согласись или не согласись с конкретным тезисом, дополни своей мыслью, или задай уточняющий вопрос"
