...
def _scope_label(scope_name: str, lang: str = "ru") -> str:
    return {
        ...
        "text_analysis": "Анализ текстов (ответы на задания из LMS, посты в клубе, оценка мировоззрения)",
...
def _activity_summary(events: dict[str, int]) -> str:
    ...
    if total == 0:
        return (
            ...
            '  • <b>Занятия и тренировки</b> — /learn, /train в боте\n'
    ...
    return (
        ...
        f"  • Развитие: {events['learning']} событий\n\n"
...
def _privacy_text() -> str:
    return (
        ...
        "  • Не анализируем тексты без твоего отдельного согласия (см. «Анализ текстов» ниже)\n\n"
...
_NOT_LINKED_TEXT = (
    ...
    "(занятия, практика, заметки) принадлежат именно тебе. Это даёт привязка к Aisystant.\n\n"
...
_LINKED_BUT_SYNCING_TEXT = (
    ...
    "Если записи для входа Ory ещё нет, запрос на её создание по email твоего "
    "Aisystant-аккаунта уже поставлен в очередь. Бот не получает и не создаёт пароль. "
    "При работающей автоматической обработке это обычно занимает меньше минуты.\n\n"
    "Бот никогда не просит пересылать коды или письма. Уже входил через Aisystant? "
    "Можно использовать официальный вход ниже. Если ожидание затянулось, нажми "
    "«🔄 Попробовать снова»."
)
...
async def on_consent_accept(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        "  • <b>Бот</b> — занятия (/learn), тренировки (/train), фиксации практики\n"
        "  • <b>Платформа Aisystant</b> — завершённые занятия и тренировки\n"
